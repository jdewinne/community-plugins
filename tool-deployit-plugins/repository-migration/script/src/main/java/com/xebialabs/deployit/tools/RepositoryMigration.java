...
public class RepositoryMigration {
    ...
    private void migrate() throws Exception {
        ...
        if (updateConfiguration) {
            ...
            System.out.println(" backup the previous configuration file " + deployitConfigurationFile);
            final File deployitConfBackupFile = new File(deployitConfigurationFile.getParentFile(), deployitConfigurationFile.getName() + "." + timestamp);
            ...
            deployitProperties.store(bufferedWriter, "Generated by XL Repository Migration Tools");

        }
        System.out.println("Done in " + ((stop - start) / 1000) + " seconds");
    ...
    private File getDeployitConfigurationFile(final File sourceDir) {
        File confFile = new File(new File(sourceDir, "conf"), "deployit.conf");
        if (!confFile.exists()) {
            // try XL Release configuration
            confFile = new File(new File(sourceDir, "conf"), "xl-release-server.conf");
        }
        if (!confFile.exists()) {
            throw new IllegalStateException("Could not find neither XL Deploy (deployit.conf) nor XL Release " +
                "(xl-release-server.conf) configuration file in conf/ folder");
        }
        return confFile;
    }
